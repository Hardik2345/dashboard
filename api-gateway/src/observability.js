...
const SERVICE_NAME = process.env.SERVICE_NAME || "api-gateway-auth";
...
let appErrors = null;
let authLoginTotal = null;
let authRefreshTotal = null;
let authTokenReuseTotal = null;
...
function initMetrics() {
  if (!promClient || !METRICS_ENABLED) return;
  ...
  authLoginTotal = new promClient.Counter({
    name: "auth_login_total",
    help: "Authentication login attempts by result.",
    labelNames: ["result"],
  });
  authRefreshTotal = new promClient.Counter({
    name: "auth_refresh_total",
    help: "Authentication refresh attempts by result.",
    labelNames: ["result"],
  });
  authTokenReuseTotal = new promClient.Counter({
...
function captureError(err, req, extra = {}) {
  ...
  Sentry.withScope((scope) => {
    ...
    if (req) {
      ...
      const brandKey = req.headers?.["x-brand-id"] || req.query?.brand_key;
...
function recordAuthLogin(result) {
  if (authLoginTotal) authLoginTotal.inc({ result });
}

function recordAuthRefresh(result) {
  if (authRefreshTotal) authRefreshTotal.inc({ result });
}

function recordAuthTokenReuse() {
  if (authTokenReuseTotal) authTokenReuseTotal.inc();
}
