...
/**
 * Build an S3 glob pattern for a date range.
 * Structure: s3://bucket/prefix/brand_key/year=YYYY/month=MM/day=DD/data.parquet
 */
function buildS3GlobForRange(bucket, brandKey, startDate, endDate) {
  const prefix = getS3Prefix();
  // Use glob pattern spanning all matching partitions
...
async function queryCold({ brandKey, startDate, endDate, filters = {} }) {
  ...
  try {
    ...
    // If the parquet files don't exist yet, return empty
...
async function queryHot(conn, { startDate, endDate, filters = {} }) {
  ...
  // Support both Sequelize-like { query(sql, opts) } and mysql2 pool
  ...
  // Sequelize-like connection (from tenantConnection.js)
...
async function writeParquetToS3(rows, s3Path) {
  ...
  // Create a temp table, insert rows, then COPY to S3
