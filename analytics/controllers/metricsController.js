...
async function calcTotalSalesDelta({
  ...
  if (compare === "prev-range-avg" && start && end) {
    ...
    const prevAvg = await avgForRange("total_sales", {
...
async function calcTotalSessionsDelta({
  ...
  if (hasUtmFilters(filters)) {
    const rangeStart = start || date;
    const rangeEnd = end || date;
    const curr = await computeTotalSessions({
      start: rangeStart,
      end: rangeEnd,
      conn,
      filters,
    ...
    const prevWin = previousWindow(rangeStart, rangeEnd);
    const prev = await computeTotalSessions({
      start: prevWin.prevStart,
      end: prevWin.prevEnd,
      conn,
      filters,
  ...
  if (compare === "prev-range-avg" && start && end) {
    ...
    const prevWin = previousWindow(start, end);
    const prevAvg = await avgForRange("total_sessions", {
      start: prevWin.prevStart,
      end: prevWin.prevEnd,
      conn,
    });
  ...
  if (alignLower === "hour") {
    ...
    if (start && end) {
      ...
      if (filters?.device_type) {
        const curr = await computeSessionsFromDeviceColumns({
          start,
          end,
          conn,
          filters,
        ...
        const prev = await computeSessionsFromDeviceColumns({
          start: prevWin.prevStart,
          end: prevWin.prevEnd,
          conn,
          filters,
...
async function calcAtcSessionsDelta({
  ...
  if (hasUtmFilters(filters)) {
    const rangeStart = start || date;
    const rangeEnd = end || date;
    const curr = await computeAtcSessions({
      start: rangeStart,
      end: rangeEnd,
      conn,
      filters,
    ...
    const prevWin = previousWindow(rangeStart, rangeEnd);
    const prev = await computeAtcSessions({
      start: prevWin.prevStart,
      end: prevWin.prevEnd,
      conn,
      filters,
  ...
  if (compare === "prev-range-avg" && start && end) {
    const currAvg = await avgForRange("total_atc_sessions", {
      start,
      end,
      conn,
    });
    const prevWin = previousWindow(start, end);
    const prevAvg = await avgForRange("total_atc_sessions", {
      start: prevWin.prevStart,
      end: prevWin.prevEnd,
      conn,
    });
...
async function calcCvrDelta({ start, end, align, compare, conn, filters }) {
  ...
  if (hasUtmFilters(filters)) {
    ...
    if (start && end) {
      ...
      const prevWin = previousWindow(start, end);
      prev = await cvrForRange({
        start: prevWin.prevStart,
        end: prevWin.prevEnd,
        conn,
        filters,
      });
      const delta = computePercentDelta(
        curr.cvr_percent || 0,
        prev.cvr_percent || 0,
      );
      // Handle 0 sessions
  ...
  if (alignLower === "hour") {
    ...
    if (start && end) {
      ...
      const rangeEnd = end;
      const targetHour = resolveTargetHour
        ? resolveTargetHour(end)
        : end === todayIst
          ? nowIst.getUTCHours()
          : 23;
      const effectiveSeconds = Math.min(
      ...
      const [
        ...
        isToday(rangeStart) || isToday(rangeEnd)
          ? conn.query(sqlOverallSessions, {
              type: QueryTypes.SELECT,
...
async function fetchCachedMetrics(brandKey, date) {
  ...
  if (MEM_CACHE.has(key)) {
    ...
    if (now - entry.timestamp < CACHE_TTL_MS) {
      if (entry.promise) {
        logger.debug(
          `[MEM CACHE] Reuse pending request for ${brandKey} on ${date}`,
        );
...
function buildMetricsController() {
  return {
            ...
            await computeMetricDelta({
              ...
              queryFn: async ({
                ...
                const [currRows, prevRows] = await Promise.all([
                  req.brandDb.sequelize.query(sqlRange, {
                    type: QueryTypes.SELECT,
                    replacements: curReplacements,
                  }),
                  req.brandDb.sequelize.query(sqlRange, {
                ...
                return {
                  currentVal: Number(currRows?.[0]?.total || 0),
                  previousVal: Number(prevRows?.[0]?.total || 0),
                };
              },
            }),
          );
    ...
    rolling30d: async (req, res) => {
      try {
        const brandQuery =
          (req.query.brand || req.query.brand_key || "").toString().trim() ||
          null;
        let conn =
          req.brandDb && req.brandDb.sequelize ? req.brandDb.sequelize : null;
        if (brandQuery) {
        ...
        if (end) {
          ...
          if (!parsed.success)
            return res
              .status(400)
              .json({ error: "Invalid end date. Use YYYY-MM-DD" });
        } else {
          const rows = await conn.query(
            "SELECT MAX(date) AS max_d FROM overall_summary",
            { type: QueryTypes.SELECT },
          );
          const maxd = Array.isArray(rows)
            ? rows[0]?.max_d
            : rows && rows.max_d;
        ...
        for (const day of days) {
          ...
          const winEnd = day;
          const aovVal = await aovForRange({
            start: winStart,
            end: winEnd,
            conn,
          });
          const aovTotals = await computeAOV({
            start: winStart,
            end: winEnd,
            conn,
          });
          const cvrObj = await cvrForRange({
            start: winStart,
            end: winEnd,
            conn,
          });
          const cvrTotals = await computeCVR({
            start: winStart,
            end: winEnd,
            conn,
          });
          ...
          series.push({
            ...
            aov_totals: {
              total_sales: Number(aovTotals.total_sales || 0),
              total_orders: Number(aovTotals.total_orders || 0),
            },
            cvr_30d: Number(cvrObj.cvr || 0),
            cvr_percent_30d: Number(cvrObj.cvr_percent || 0),
            cvr_totals: {
              total_orders: Number(cvrTotals.total_orders || 0),
              total_sessions: Number(cvrTotals.total_sessions || 0),
            },
          });
        }
            ...
            await computeMetricDelta({
              ...
              queryFn: async ({
                ...
                const [currRow, prevRow, overallCurrRow] = await Promise.all([
                  ...
                  req.brandDb.sequelize.query(sqlRange, {
                    type: QueryTypes.SELECT,
                    replacements: [prevStart, prevEnd, prevCompareHour],
                  }),
                  isCurrentRangeToday
                    ? req.brandDb.sequelize.query(sqlOverallSessions, {
                        type: QueryTypes.SELECT,
                        replacements: [rangeStart, rangeEnd],
                      })
                    : Promise.resolve(null),
            ...
            await computeMetricDelta({
              ...
              queryFn: async ({
                ...
                const [currRow, prevRow, overallCurrRow] = await Promise.all([
                  ...
                  isCurrentRangeToday
                    ? req.brandDb.sequelize.query(sqlOverallAtc, {
                        type: QueryTypes.SELECT,
                        replacements: [rangeStart, rangeEnd],
                      })
                    : Promise.resolve(null),
          ...
          return res.json(
            await computeMetricDelta({
              ...
              queryFn: async ({
                ...
                if (filters) {
                  salesSqlRange = appendUtmWhere(
                    salesSqlRange,
                    curReplacements,
                    filters,
                  );
                  ordersSqlRange = appendUtmWhere(
                    ordersSqlRange,
                    curReplacements,
                    filters,
                  );
                ...
                const [
                  ...
                  req.brandDb.sequelize.query(salesSqlRange, {
                    type: QueryTypes.SELECT,
                    replacements: curReplacements,
                  }),
                  req.brandDb.sequelize.query(salesSqlRange, {
                  ...
                  req.brandDb.sequelize.query(ordersSqlRange, {
                    type: QueryTypes.SELECT,
                    replacements: curReplacements,
                  }),
                  req.brandDb.sequelize.query(ordersSqlRange, {
    ...
    trafficSourceSplit: async (req, res) => {
      try {
        ...
        console.log(
          `[TrafficSplit] Current: ${start} to ${end}, Previous: ${pStart} to ${pEnd}`,
        );
    ...
    paymentSalesSplit: async (req, res) => {
      ...
      GROUP BY payment_type`;
        ...
        try {
          ...
          rows = await req.brandDb.sequelize.query(sql, {
            type: QueryTypes.SELECT,
            replacements,
          });
        } catch (e) {
        ...
        for (const r of rows) {
          if (r.payment_type === "COD") cod_sales = Number(r.sales || 0);
            ...
            prepaid_sales = Number(r.sales || 0);
          else if (r.payment_type === "Partial")
            partial_sales = Number(r.sales || 0);
        }
    ...
    hourlyTrend: async (req, res) => {
      try {
       ...
       WHERE date >= ? AND date <= ?`;
        ...
        const rows = await req.brandDb.sequelize.query(querySql, {
          type: QueryTypes.SELECT,
        ...
        if (filters.product_id) {
          try {
            ...
            const sessionRows = await req.brandDb.sequelize.query(sessionSql, {
              type: QueryTypes.SELECT,
              replacements: sessionReplacements,
            });
        ...
        for (const row of rows) {
          ...
          const dateStr =
            row.date instanceof Date
              ...
              : String(row.date);
          const hourVal =
            typeof row.hour === "number" ? row.hour : Number(row.hour);
          if (!Number.isFinite(hourVal) || hourVal < 0 || hourVal > 23)
            continue;
          ...
          if (!filters.product_id) {
            ...
            existing.adjusted_sessions += Number(
              row.adjusted_number_of_sessions || 0,
            );
        ...
        if (filters.device_type && !filters.product_id) {
          try {
            const types = Array.isArray(filters.device_type)
              ? filters.device_type
              : [filters.device_type];
            const sessCols = [];
            const atcCols = [];
            ...
            if (sessCols.length > 0) {
              const sessExpr = sessCols
                .map((c) => `COALESCE(${c}, 0)`)
                .join(" + ");
              const atcExpr = atcCols
                .map((c) => `COALESCE(${c}, 0)`)
                .join(" + ");
              const deviceSessionSql = `SELECT DATE_FORMAT(date, '%Y-%m-%d') AS date, hour,
                ...
                WHERE date >= ? AND date <= ?
                GROUP BY date, hour`;
              const deviceRows = await req.brandDb.sequelize.query(
                deviceSessionSql,
                {
                  type: QueryTypes.SELECT,
                  replacements: [start, end],
              ...
              for (const dr of deviceRows) {
                if (!dr?.date) continue;
                const dateStr =
                  dr.date instanceof Date
                    ? dr.date.toISOString().slice(0, 10)
                    : String(dr.date);
                const hourVal =
                  typeof dr.hour === "number" ? dr.hour : Number(dr.hour);
                if (!Number.isFinite(hourVal) || hourVal < 0 || hourVal > 23)
                  continue;
                const key = `${dateStr}#${hourVal}`;
                const existing = rowMap.get(key) || {
            ...
            console.warn(
              "[hourlyTrend] Failed to fetch device-type sessions",
              err,
            );
          }
        }
        ...
        if (
          ...
          for (const { date: d, hour } of buckets) {
            const metrics = rowMap.get(`${d}#${hour}`) || {
              sales: 0,
              sessions: 0,
              orders: 0,
              atc: 0,
          ...
          points = Array.from({ length: maxHourForSeries + 1 }, (_, hour) => {
            ...
            const avgAdjustedSessions = acc.count
              ? acc.adjusted_sessions / acc.count
              : 0;
          ...
          points = buckets.map(({ date: bucketDate, hour }) => {
            const metrics = rowMap.get(`${bucketDate}#${hour}`) || {
              sales: 0,
              sessions: 0,
              orders: 0,
              atc: 0,
            ...
            const cvrRatio =
              metrics.sessions > 0 ? metrics.orders / metrics.sessions : 0;
            const monthIndex = Math.max(
              0,
              Math.min(11, Number(bucketDate.slice(5, 7)) - 1),
            );
    ...
    number_of_atc_sessions
         ...
         WHERE date >= ? AND date <= ?`;
          ...
          const comparisonRows = await req.brandDb.sequelize.query(compSql, {
            type: QueryTypes.SELECT,
            replacements: compReplacements,
          });
          ...
          for (
            let ts = parseIsoDate(prevWin.prevStart).getTime();
            ts <= parseIsoDate(prevWin.prevEnd).getTime();
            ts += DAY_MS
          ) {
            const dt = new Date(ts);
            const yyyy = dt.getUTCFullYear();
            ...
            const dateStr = `${yyyy}-${mm}-${dd}`;
            const maxHour =
              dateStr === prevWin.prevEnd ? comparisonAlignHour : 23;
            for (let hour = 0; hour <= maxHour; hour += 1)
          ...
          for (const { date: bucketDate, hour } of comparisonBuckets) {
            const metrics = comparisonRowMap.get(`${bucketDate}#${hour}`) || {
              sales: 0,
              sessions: 0,
              orders: 0,
              atc: 0,
      ...
      try {
        ...
        if (filters.product_id) {
          try {
            ...
            sessionRows.forEach((r) => {
              productSessionsMap.set(String(r.date), {
        ...
        if (hasFilters && !filters.product_id) {
          try {
            ...
            const utmSessRows = await req.brandDb.sequelize.query(utmSessSql, {
              type: QueryTypes.SELECT,
              replacements: utmSessReplacements,
            });
            utmSessRows.forEach((r) => {
              utmSessionsMap.set(String(r.date), {
                sessions: Number(r.sessions || 0),
                atc: Number(r.atc || 0),
              });
            });
        ...
        const map = new Map(
          rows.map((r) => {
            ...
            if (filters.product_id) {
              ...
              const productData = productSessionsMap.get(d) || {
                sessions: 0,
                atc: 0,
              };
          ...
          overallMap = new Map(
            overallRows.map((r) => {
              ...
              return [
                ...
                {
                  total_sessions: Number(r.total_sessions || 0),
                  adjusted_total_sessions:
                    r.adjusted_total_sessions == null
                      ? null
                      : Number(r.adjusted_total_sessions),
                },
              ];
            }),
          );
        ...
        const points = dayList.map((d) => {
          ...
          return {
            ...
            metrics: {
              ...
              adjusted_sessions: metrics.adjusted_sessions,
              raw_sessions: metrics.raw_sessions,
              atc: bestAtc,
              cvr_ratio: cvrRatio,
              cvr_percent: cvrRatio * 100,
            },
        ...
        if (explicitCompStartD && explicitCompEndD) {
          prevWin = {
            prevStart: explicitCompStartD,
            prevEnd: explicitCompEndD,
          };
        } else {
          prevWin = previousWindow(start, end);
        }
        if (prevWin?.prevStart && prevWin?.prevEnd) {
          ...
          if (hasFilters && !filters.product_id) {
            try {
              ...
              const compUtmSessReplacements = [
                prevWin.prevStart,
                prevWin.prevEnd,
              ];
              compUtmSessSql = appendUtmWhere(
                compUtmSessSql,
                compUtmSessReplacements,
                { ...filters, sales_channel: null },
                true,
              );
              compUtmSessSql += ` GROUP BY date`;

              const compUtmSessRows = await req.brandDb.sequelize.query(
                compUtmSessSql,
                {
                  type: QueryTypes.SELECT,
                  replacements: compUtmSessReplacements,
                },
              );
              compUtmSessRows.forEach((r) => {
                compUtmSessionsMap.set(String(r.date), {
                  sessions: Number(r.sessions || 0),
                  atc: Number(r.atc || 0),
                });
              });
              ...
              console.warn(
                "[dailyTrend] Failed to fetch comparison UTM sessions",
                err,
              );
            }
          }

          const rowsPrev = await req.brandDb.sequelize.query(compSql, {
            type: QueryTypes.SELECT,
            replacements: compReplacements,
          });
          ...
          if (filters.product_id) {
            try {
              ...
              const sessionRows = await req.brandDb.sequelize.query(
                sessionSql,
                { type: QueryTypes.SELECT, replacements: sessionReplacements },
              );
              sessionRows.forEach((r) => {
                compProductSessionsMap.set(String(r.date), {
                  sessions: Number(r.sessions || 0),
                  atc: Number(r.atc || 0),
                });
              });
              ...
              console.warn(
                "[dailyTrend] Failed to fetch comparison product sessions",
                err,
              );
            }
          }
          ...
          const mapPrev = new Map(
            rowsPrev.map((r) => {
              ...
              if (filters.product_id) {
                ...
                const productData = compProductSessionsMap.get(d) || {
                  sessions: 0,
                  atc: 0,
                };
                sessions = productData.sessions;
                ...
                // Use UTM sessions
                const utmData = compUtmSessionsMap.get(d) || {
                  sessions: 0,
                  atc: 0,
                };
                sessions = utmData.sessions;
          ...
          if (!hasFilters) {
            const overallRowsPrev = await req.brandDb.sequelize.query(
              ...
              {
                type: QueryTypes.SELECT,
                replacements: [prevWin.prevStart, prevWin.prevEnd],
              },
            );
            overallMapPrev = new Map(
              overallRowsPrev.map((r) => {
                const d = String(r.date);
                return [
                  d,
                  {
        ...
        return res.json({
          ...
          comparison: comparison
            ? { ...comparison, days: comparison.points }
            : null,
        });
      } catch (e) {
    ...
    monthlyTrend: async (req, res) => {
      try {
        ...
        const [rows, sessionsRows] = await Promise.all([
          req.brandDb.sequelize.query(sql, {
            type: QueryTypes.SELECT,
            replacements: replacements,
          }),
          sessionsSql
            ? req.brandDb.sequelize.query(sessionsSql, {
                type: QueryTypes.SELECT,
                replacements: sessionsReplacements,
              })
            : Promise.resolve([]),
        ]);

        const sessionsMap = new Map(
          sessionsRows.map((r) => [r.month_start, r]),
        );

        const points = rows.map((r) => {
          const s = sessionsMap.get(r.month_start);
          const bestSession =
            s && s.adjusted_total_sessions != null
              ? Number(s.adjusted_total_sessions)
        ...
        if (explicitCompStartM && explicitCompEndM) {
          prevWin = {
            prevStart: explicitCompStartM,
            prevEnd: explicitCompEndM,
          };
        } else {
          prevWin = previousWindow(start, end);
        }
        if (prevWin?.prevStart && prevWin?.prevEnd) {
          ...
          const [rowsPrev, sessionsRowsPrev] = await Promise.all([
            req.brandDb.sequelize.query(compSql, {
              type: QueryTypes.SELECT,
              replacements: compReplacements,
            }),
            compSessSql
              ? req.brandDb.sequelize.query(compSessSql, {
                  type: QueryTypes.SELECT,
                  replacements: compSessReplacements,
                })
              : Promise.resolve([]),
          ]);
          const sessionsMapPrev = new Map(
            sessionsRowsPrev.map((r) => [r.month_start, r]),
          );
        ...
        return res.json({
          ...
          months: points,
          comparison: comparison
            ? { ...comparison, months: comparison.points }
            : null,
        });
      } catch (e) {
    ...
    productConversion: async (req, res) => {
      try {
        ...
        const sortBy = (req.query.sort_by || "sessions")
          .toString()
          .toLowerCase();
        ...
        if (compareStart && compareEnd && rows.length > 0) {
          ...
          if (productIds.length > 0 || paths.length > 0) {
            try {
              ...
              if (paths.length > 0) {
                console.log(
                  "[DEBUG] Fetching comparison sessions for paths:",
                  paths.length,
                  "Start:",
                  compareStart,
                  "End:",
                ...
                if (sessionsRows.length > 0) {
                  ...
                  console.log(
                    "[DEBUG] No session rows found for paths:",
              ...
              rows = rows.map((r) => {
                ...
                const atcRate =
                  pSessions.sessions > 0
                    ? (pSessions.atc / pSessions.sessions) * 100
                    : 0;
                const cvr =
                  pSessions.sessions > 0
                    ? (pOrders.orders / pSessions.sessions) * 100
                    : 0;
              ...
              console.error("[product-conversion] comparison logic failed", e);
            }
          }
        }
    ...
    productConversionCsv: async (req, res) => {
      try {
        ...
        const fullSql = `
          ...
          ORDER BY ${sortCol} ${dir}
        `;
        const csvRows = await conn.query(fullSql, {
          type: QueryTypes.SELECT,
          replacements,
        });
        ...
        if (Array.isArray(visibleColumns) && visibleColumns.length > 0) {
          finalHeaders = allHeaders.filter(
            (h) => visibleColumns.includes(h) || h === "landing_page_path",
          );
        }
        ...
        for (const r of csvRows) {
          const rowVals = finalHeaders.map((h) => {
            ...
            if (h === "landing_page_path")
              return escapeCsv(r.landing_page_path);
            return Number(r[h] || 0);
          });
    ...
    hourlySalesCompare: async (req, res) => {
      try {
        ...
        const [rowsCurrent, rowsY] = await Promise.all([
          brandConn.sequelize.query(sql, {
            type: QueryTypes.SELECT,
            replacements: paramsCurrent,
          }),
          brandConn.sequelize.query(sql, {
            type: QueryTypes.SELECT,
            replacements: paramsY,
          }),
        ]);
        ...
        const labels = bucketsIst.map(
          (b) => `${String(b.hour).padStart(2, "0")}:00`,
        );
        const current = bucketsIst.map(
          (b) => mapCurrent.get(`${b.date}#${b.hour}`) || 0,
        );
        const yesterday = yBucketsIst.map(
          (b) => mapY.get(`${b.date}#${b.hour}`) || 0,
        );
      ...
      try {
        ...
        if (redisClient) {
          try {
            ...
            console.error(
              "[hourlySalesSummary] Redis fetch failed",
              err.message,
            );
          }
        }
        ...
        if (!todayData || !yesterdayData) {
          ...
          if (!todayData) {
            ...
            logger.debug(
              `[DB FETCH] hourly sales for ${brandKey} on ${todayStr}`,
            );
          }
          ...
          if (!yesterdayData) {
            ...
            logger.debug(
              `[DB FETCH] hourly sales for ${brandKey} on ${yesterdayStr}`,
            );
          }
        }
    ...
    deltaSummary: async (req, res) => {
      try {
        ...
        if (isSingleDay && !hasFilters && !align && !compare) {
          try {
            const prevWin = previousWindow(start, end);
            const [cached, cachedPrev] = await fetchCachedMetricsBatch(
              brandQuery,
              [start, prevWin.prevStart],
            );
            ...
            if (cached && cachedPrev) {
              const calcDeltaLocal = (cur, prev) => {
                const diff = cur - prev;
                const diff_pct =
                  prev > 0 ? (diff / prev) * 100 : cur > 0 ? 100 : 0;
                const direction =
                  diff > 0.0001 ? "up" : diff < -0.0001 ? "down" : "flat";
              ...
              usedCache = true;
              logger.debug(
                `[deltaSummary] Served from cache for ${brandQuery} ${start}`,
              );
            }
          } catch (e) {
        ...
        const response = {
          range: { start, end },
          prev_range: prevWin
            ? { start: prevWin.prevStart, end: prevWin.prevEnd }
            : null,
          metrics: {
            total_orders: orders,
            total_sales: sales,
    ...
    dashboardSummary: async (req, res) => {
      try {
        ...
        const explicitCompareStart = req.query.compare_start
          ? req.query.compare_start.toString()
          : null;
        const explicitCompareEnd = req.query.compare_end
          ? req.query.compare_end.toString()
          : null;
        const hasExplicitCompare = !!(
          explicitCompareStart && explicitCompareEnd
        );
        ...
        const mark = (label, since) =>
          spans.push({ label, ms: Date.now() - (since || traceStart) });
        mark("params");
        ...
        logger.debug(
          `[SUMMARY] Fetching for ${brandQuery} range ${start} to ${end}${prevStart && prevEnd ? ` (prev: ${prevStart} to ${prevEnd})` : ""}${hasExplicitCompare ? " [EXPLICIT COMPARE]" : ""}`,
        );
        ...
        const [cached, cachedPrev] =
          isSingleDay && !hasFilters && !hasExplicitCompare
            ? await fetchCachedMetricsBatch(brandQuery, [
                start,
                prevStart || prevDayStr(start),
              ])
            : [null, null];
        ...
        const getMetricsForRange = async (s, e, cachedData) => {
          ...
          if (!req.brandDb && req.brandConfig) {
            logger.debug(
              `[LAZY CONNECT] Connecting to ${req.brandConfig.key} for fallback`,
            );
            try {
              req.brandDbName = req.brandConfig.dbName || req.brandConfig.key;
            } catch (connErr) {
        ...
        const currentRangeIncludesToday =
          start <= todayIstStr && end >= todayIstStr;
        ...
        if (hasExplicitCompare) {
          ...
          const getMetricsWithCutoff = async (s, e, cutoff) => {
            ...
            if (filters)
              orderSql = appendUtmWhere(orderSql, orderRepl, filters);
            const orderRows = await conn.query(orderSql, {
              type: QueryTypes.SELECT,
              replacements: orderRepl,
            });
            ...
            if (filters)
              salesSql = appendUtmWhere(salesSql, salesRepl, filters);
            const salesRows = await conn.query(salesSql, {
              type: QueryTypes.SELECT,
              replacements: salesRepl,
            });
            ...
            if (s === e) {
              ...
              const sessRows = await conn.query(sessSql, {
                type: QueryTypes.SELECT,
                replacements: [s, hourCutoff],
              });
              const atcRows = await conn.query(atcSql, {
                type: QueryTypes.SELECT,
                replacements: [s, hourCutoff],
              });
          ...
          const buildMetric = (currVal, prevVal) => {
            ...
            const diff_pct =
              prevVal > 0 ? (diff / prevVal) * 100 : currVal > 0 ? 100 : 0;
            const direction =
              diff > 0.0001 ? "up" : diff < -0.0001 ? "down" : "flat";
            return {
              value: currVal,
          ...
          total_orders = buildMetric(
            currMetrics.total_orders,
            prevMetrics.total_orders,
          );
          total_sales = buildMetric(
            currMetrics.total_sales,
            prevMetrics.total_sales,
          );
          total_sessions = buildMetric(
            currMetrics.total_sessions,
            prevMetrics.total_sessions,
          );
          ...
          // AOV
          const currAov =
            currMetrics.total_orders > 0
              ? currMetrics.total_sales / currMetrics.total_orders
              : 0;
          const prevAov =
            prevMetrics.total_orders > 0
              ? prevMetrics.total_sales / prevMetrics.total_orders
              : 0;
          average_order_value = buildMetric(currAov, prevAov);

          // CVR
          const currCvr =
            currMetrics.total_sessions > 0
              ? (currMetrics.total_orders / currMetrics.total_sessions) * 100
              : 0;
          const prevCvr =
            prevMetrics.total_sessions > 0
              ? (prevMetrics.total_orders / prevMetrics.total_sessions) * 100
              : 0;
          ...
          sources = {
            ...
            hourly_cutoff: needsHourlyCutoff ? cutoffTime : null,
          };
        } else if (isTodayIst) {
          ...
          if (filters.sales_channel) {
            ...
            logger.debug(
              `[HYBRID DELTA] Fetching global deltas (ignoring sales_channel: ${filters.sales_channel})`,
            );
          ...
          const [current, previous] = await Promise.all([
            ...
            (async () => {
              const s = Date.now();
              const resPrev = await getMetricsForRange(
                prevStart,
                prevEnd,
                cachedPrev,
              );
          ...
          if (filters.sales_channel) {
            ...
            logger.debug(
              `[HYBRID DELTA] Fetching global full-day metrics for delta override`,
            );
          ...
          const calcHybrid = (key, primCur, primPrev, globCur, globPrev) => {
            ...
            const direction =
              diffG > 0.0001 ? "up" : diffG < -0.0001 ? "down" : "flat";

            return { value: valC, previous: valP, diff, diff_pct, direction };
          };

          total_orders = calcHybrid(
            "total_orders",
            current,
            previous,
            currentGlobal,
          ...
          average_order_value = calcHybrid(
            "average_order_value",
            current,
            previous,
            currentGlobal,
        ...
        if (req.query.include_utm_options === "true") {
          ...
          if (!req.brandDb && req.brandConfig) {
            try {
              ...
              console.error(
                "[dashboardSummary] Lazy connection for UTM options failed",
                connErr,
              );
            }
          }
          ...
          if (req.brandDb) {
            ...
            const getSourceFilter = () => {
              if (filters.utm_source) {
                const vals = Array.isArray(filters.utm_source)
                  ? filters.utm_source
                  : [filters.utm_source];
                if (vals.length > 0) {
            ...
            for (const row of utmRows) {
              ...
              if (m) {
                if (!utmTree[s].mediums[m])
                  utmTree[s].mediums[m] = { campaigns: {} };
                if (c) {
                  if (!utmTree[s].mediums[m].campaigns[c])
                    utmTree[s].mediums[m].campaigns[c] = {
                      terms: [],
                      contents: [],
                    };
                  if (
                    t &&
                    !utmTree[s].mediums[m].campaigns[c].terms.includes(t)
                  ) {
                    utmTree[s].mediums[m].campaigns[c].terms.push(t);
                  }
                  if (
                    ct &&
                    !utmTree[s].mediums[m].campaigns[c].contents.includes(ct)
                  ) {
