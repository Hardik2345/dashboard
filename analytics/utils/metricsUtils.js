...
async function computeCVR({ start, end, conn, filters }) {
  ...
  const total_sessions = await computeTotalSessions({
    start,
    end,
    conn,
    filters,
...
async function computeCVRForDay(date, conn, filters = {}) {
  if (!date)
    return { total_orders: 0, total_sessions: 0, cvr: 0, cvr_percent: 0 };
...
async function computeTotalSessions({ start, end, conn, filters }) {
  if (filters?.device_type) {
    return computeSessionsFromDeviceColumns({
      start,
      end,
      conn,
      filters,
  ...
  if (hasUtmFilters(filters)) {
    return computeSessionsFromSnapshot({
      start,
      end,
      conn,
      filters,
...
async function computeAtcSessions({ start, end, conn, filters }) {
  ...
  if (hasUtmFilters(filters)) {
    return computeSessionsFromSnapshot({
      ...
      conn,
      filters,
      column: "sessions_with_cart_additions",
    });
  }
...
async function computeSessionsFromSnapshot({
  ...
  const snapRows = await conn.query(snapshotSql, {
    type: QueryTypes.SELECT,
    replacements: snapParams,
  });
...
async function aovForRange({ start, end, conn, filters }) {
  if (!start || !end) return 0;
  const { total_sales, total_orders } = await computeAOV({
    start,
    end,
    conn,
    filters,
...
async function cvrForRange({ start, end, conn, filters }) {
  ...
  const { total_orders, total_sessions } = await computeCVR({
    start,
    end,
    conn,
    filters,
 ...
 */
async function computeSessionsFromDeviceColumns({
  start,
  end,
  conn,
  filters,
