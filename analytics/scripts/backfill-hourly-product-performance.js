...
import { loadPipelineBrandsForScripts } from "./utils/brandLoader.js";
...
async function fetchHourlyDimensionSessions(brand, targetYmd) {
  const allHoursRows = await runShopifyQL(
    brand,
    buildShopifyQLHourlyDimensionAllHoursQuery(targetYmd),
    "hourly-dim(all)",
  );

  if (allHoursRows.length < 1000) {
    return allHoursRows
      .map((row) => ({
        ...row,
  ...
  for (let hour = 0; hour < 24; hour += 1) {
    const rows = await runShopifyQL(
      brand,
      buildShopifyQLHourlyDimensionQuery(targetYmd, hour),
      `hourly-dim(${hour})`,
    );
    for (const row of rows) {
      out.push({ ...row, hour });
