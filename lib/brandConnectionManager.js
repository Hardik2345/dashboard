...
// Conservative defaults: limit connections to prevent exhaustion (especially with RDS Proxy)
// Total max connections = LRU_MAX * POOL_MAX (e.g., 3 brands * 1 conn = 3)
const LRU_MAX = Number(process.env.BRAND_CONN_MAX || 1);
const POOL_MAX = Number(process.env.BRAND_POOL_MAX || 1);  // keep tiny, proxy will multiplex
const POOL_MIN = Number(process.env.BRAND_POOL_MIN || 0); // 0 = release all idle connections
const POOL_IDLE = Number(process.env.BRAND_POOL_IDLE || 2000); // 2 seconds idle before release
const POOL_ACQUIRE = Number(process.env.BRAND_POOL_ACQUIRE || 30000);
const POOL_EVICT = Number(process.env.BRAND_POOL_EVICT || 1000);
// Hard cap on how long we keep an unused pool in the cache before closing it.
// Keeps rapid brand switching from leaving behind lots of sleepy connections.
const CACHE_MAX_IDLE_MS = Number(process.env.BRAND_CONN_IDLE_MS || 5_000); // 5s
...
async function createConnection(cfg) {
  ...
  const sequelize = new Sequelize(cfg.dbName, cfg.dbUser, cfg.dbPass, {
    host: cfg.proxyHost || process.env.BRAND_PROXY_HOST || cfg.dbHost,
    port: Number(cfg.proxyPort || process.env.BRAND_PROXY_PORT || cfg.dbPort || 3306),
    dialect: 'mysql',
...
async function pruneStale(now = Date.now()) {
  ...
  for (const [key, entry] of cache.entries()) {
    if (now - entry.lastUsed > CACHE_MAX_IDLE_MS) {
  ...
  for (const [key, entry] of stale) {
    ...
    try {
      ...
      console.error(`[brandConnectionManager] Failed to close stale pool for ${key}:`, e.message);
    }
  }
}
