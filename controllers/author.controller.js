...
const { getBrands, addBrandRuntime } = require('../config/brands');
...
// Simple in-module lock to serialize brand persistence operations
const brandPersistLock = { locked: false };
async function withBrandLock(fn) {
  while (brandPersistLock.locked) { // naive spin-wait with backoff
...
const AuthorController = {
  async listBrands(req, res) {
    ...
    try {
      const brands = Object.values(getBrands()).map(b => ({ key: b.key, host: b.dbHost, db: b.dbName }));
      return res.json({ brands });
    } catch (e) {
      console.error('[author.listBrands] failed', e);
