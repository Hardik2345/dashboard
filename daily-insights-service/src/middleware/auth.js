...
// Strictly role-gated (author/admin/super_admin) — unlike requirePermission,
// no granted permission can substitute for this. Used for creating/editing
// insights, which is explicitly author/admin-only regardless of any scope a
// brand_user might be granted.
function requireAuthor(req, res, next) {
  const principal = req.principal;
  if (!principal) return res.status(401).json({ error: "missing_identity" });
  if (!principal.isAuthor) return res.status(403).json({ error: "forbidden" });
  return next();
}
