...
const resolveTenant = async (brandId) => {
    ...
    console.log(`[TenantRouter] Cache MISS/EXPIRY for brand: ${brandId}. Querying CDS...`);
    ...
    try {
        ...
        console.error(`[TenantRouter] CDS failure for brand ${brandId}:`, error.message);

        // Resilience: Cache hit (even if stale) + CDS failure -> Serve cached value
        const staleTenant = cache.getStale(brandId);
        if (staleTenant) {
            console.warn(`[TenantRouter] Serving STALE metadata for brand ${brandId} due to CDS failure`);
    ...
    if (tenant.status !== 'active') {
        console.warn(`[TenantRouter] Tenant ${brandId} status is ${tenant.status}. NOT caching.`);
