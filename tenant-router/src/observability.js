...
const SERVICE_NAME = process.env.SERVICE_NAME || "tenant-router";
...
let appErrors = null;
let tenantRouteResolveTotal = null;
let tenantRouteStaleServedTotal = null;
let tenantRouteUnavailableTotal = null;
...
function initMetrics() {
  ...
  tenantRouteResolveTotal = new promClient.Counter({
    name: "tenant_route_resolve_total",
    help: "Tenant route resolve attempts by result.",
    labelNames: ["result"],
  });
  tenantRouteStaleServedTotal = new promClient.Counter({
    name: "tenant_route_stale_served_total",
    help: "Stale tenant routes served by brand.",
    labelNames: ["brand_key"],
  });
  tenantRouteUnavailableTotal = new promClient.Counter({
    name: "tenant_route_unavailable_total",
    help: "Unavailable tenant route events by brand.",
    labelNames: ["brand_key"],
  });
...
function captureError(err, req, extra = {}) {
  ...
  Sentry.withScope((scope) => {
    ...
    if (req) {
      ...
      const brandKey = req.body?.brand_id || req.query?.brand_key || req.headers?.["x-brand-id"];
...
function recordTenantRouteResolve(result) {
  if (tenantRouteResolveTotal) tenantRouteResolveTotal.inc({ result });
}

function recordTenantRouteStaleServed(brandKey) {
  if (tenantRouteStaleServedTotal) {
    tenantRouteStaleServedTotal.inc({ brand_key: String(brandKey || "unknown").toUpperCase() });
  }
}

function recordTenantRouteUnavailable(brandKey) {
  if (tenantRouteUnavailableTotal) {
    tenantRouteUnavailableTotal.inc({ brand_key: String(brandKey || "unknown").toUpperCase() });
  }
}
