...
const SERVICE_NAME = process.env.SERVICE_NAME || "sessions-service";
...
function initMetrics() {
  ...
  sessionsCreateTotal = new promClient.Counter({
    name: "sessions_create_total",
...
function captureError(err, req, extra = {}) {
  ...
  Sentry.withScope((scope) => {
    ...
    if (req) {
      ...
      const brandKey = req.headers?.["x-brand-id"] || req.body?.brand;
...
function recordSessionCreate(result) {
  if (sessionsCreateTotal) sessionsCreateTotal.inc({ result });
}
