...
async function registerWithHealthMonitor(payload) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn("[health-monitor] registration failed", response.status, text);
      return false;
    }
    console.log("[health-monitor] registration complete", text);
    return true;
  } catch (error) {
    console.warn("[health-monitor] registration skipped", error.message);
    return false;
  }
}
