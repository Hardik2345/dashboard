...
export default function DashboardRouteContainer({
  ...
  const desktopWidgetRegistry = useMemo(
    () => ({
      ...
      kpi_trend: (
        <DeferredMount minHeight={340}>{kpiTrendDesktopElement}</DeferredMount>
      ),
      payment_split: (
        <DeferredMount minHeight={310}>{paymentSplitDesktopElement}</DeferredMount>
      ),
      payment_trend: (
        <DeferredMount minHeight={310}>{paymentTrendElement}</DeferredMount>
      ),
      traffic_split: (
        <DeferredMount minHeight={310}>{trafficSplitElement}</DeferredMount>
      ),
    }),
    [
  ...
  const mobileWidgetRegistry = useMemo(
    () => ({
      ...
      kpi_trend: (
        <DeferredMount minHeight={340}>{kpiTrendMobileElement}</DeferredMount>
      ),
      top_pages: (
        <DeferredMount minHeight={380}>{topPagesMobileElement}</DeferredMount>
      ),
      payment_split: (
        <DeferredMount minHeight={310}>{paymentSplitMobileElement}</DeferredMount>
      ),
      payment_trend: (
        <DeferredMount minHeight={310}>{paymentTrendElement}</DeferredMount>
      ),
  ...
  const extraAfterId = isMobile
    ? visibleMobileWidgetIds.includes("top_pages")
      ...
      : "kpi_trend"
    : "kpi_trend";
  const hasDashboardExtras =
    !isAuthor &&
    (hasPermission("sessions_drop_off_funnel") ||
      hasPermission("product_conversion"));
  const dashboardExtrasNode = useMemo(
    () =>
      hasDashboardExtras ? (
        <DeferredMount minHeight={550}>{renderDashboardExtras()}</DeferredMount>
      ) : null,
    [hasDashboardExtras, renderDashboardExtras],
  );
