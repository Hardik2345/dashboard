...
export default function InlineDashboardLayoutEditor({
  ...
  if (!isEditing) {
    // Plain divs here, not motion.div with `layout` — a `layout` animation
    // spring-reflows every sibling below whenever one widget's height
    // changes, which fires constantly (and looks like everything
    // "re-rendering") as DeferredMount swaps skeletons for real content
    // while the user scrolls. Reordering after a layout save just snaps
    // instead of animating, which is an acceptable trade for not fighting
    // the scroll-triggered mounts.
    return (
      <Stack spacing={{ xs: 1, md: 1 }}>
        {itemIds.flatMap((widgetId) => {
          const nodes = [
            <div key={widgetId} style={{ width: "100%" }}>
          ...
          if (extraAfterId === widgetId && extras) {
            nodes.push(
              <div key={`${widgetId}-extras`} style={{ width: "100%" }}>
                {extras}
              </div>,
            );
          }
