...
const TrendTooltip = ({ active, payload, label, formatter }) => {
  ...
  const rows = payload
    ...
    .map((entry) => ({
      ...
      name: entry.name,
      value: entry.value,
      bucket: String(entry.dataKey || "").startsWith("comparison")
        ? "Previous"
        : "Current",
    }));
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  content={<TrendTooltip formatter={config.formatter} />}
                  wrapperStyle={{ zIndex: 1000, pointerEvents: "none" }}
                />
                <Bar
                  hide={!visibleBars.includes("comparison")}
                  dataKey="comparisonPrepaid"
                  name="Prepaid"
                  stackId="comparison"
                  fill={alpha(COLORS.prepaid, 0.45)}
                  maxBarSize={28}
                />
                <Bar
                  hide={!visibleBars.includes("comparison")}
                  dataKey="comparisonCod"
                  name="COD"
                  stackId="comparison"
                  fill={alpha(COLORS.cod, 0.45)}
                  maxBarSize={28}
                />
                <Bar
                  hide={!visibleBars.includes("comparison")}
                  dataKey="comparisonPartial"
                  name="Partially paid"
                  stackId="comparison"
                  fill={alpha(COLORS.partial, 0.45)}
                  radius={[4, 4, 0, 0]}
                  maxBarSize={28}
                />
                <Bar
                  hide={!visibleBars.includes("primary")}
                  dataKey="currentPrepaid"
                  name="Prepaid"
                  stackId="current"
                  fill={COLORS.prepaid}
                  maxBarSize={28}
                ...
                >
                  <LabelList
                    ...
                    position="top"
                    formatter={config.compactFormatter}
                    fontSize={9}
                    offset={8}
