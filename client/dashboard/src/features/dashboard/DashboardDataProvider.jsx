...
// Split into two contexts on purpose. `activeRequests`/`lastError` change on
// every request start/finish, and every widget hook consumes this provider
// just for the stable get* functions — bundling them in one context value
// meant every widget re-rendered on every OTHER widget's request lifecycle.
const DashboardDataApiContext = createContext(null);
const DashboardDataStatusContext = createContext(null);
...
export function DashboardDataProvider({ children }) {
  ...
  const status = useMemo(
    ...
    [activeRequests, lastError],
  );

  return (
    <DashboardDataApiContext.Provider value={api}>
      <DashboardDataStatusContext.Provider value={status}>
        {children}
      </DashboardDataStatusContext.Provider>
    </DashboardDataApiContext.Provider>
  );
}

export function useDashboardDataApi() {
  const context = useContext(DashboardDataApiContext);
  if (!context) {
...
export function useDashboardDataStatus() {
  const context = useContext(DashboardDataStatusContext);
  if (!context) {
    throw new Error(
      "useDashboardDataStatus must be used within DashboardDataProvider",
    );
  }
  return context;
}
